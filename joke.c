...
enum SCANSTAT SCANNING = Stop;

enum SCANSTAT getScan() {
	switch ( sirc_button(PIN_REMOTE_RECV) ) {
		case PWR:
			SCANNING = Stop;
		break;
...
int main() {
	...
	while (true) {
		while ( getScan() == Stop ) {};
		if ( SCANNING == Restart ) SCANNING = Continue;

		COLOR_START(); // Just for yellow ignore the name
		...
		pause(1000); // 1 second so the user can register if it failed or not and so it can play the TTS.
